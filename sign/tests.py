...
class GuestManageTest(TestCase):
    ...
    def test_guest_manage_search_success(self):
        '''测试嘉宾搜索'''
        response = self.client.post('/login_action/',data=self.login_user)
        response = self.client.post('/search_phone/',{"phone":"13711001101"})
        self.assertEqual(response.status_code,200)
        self.assertIn(b"alen",response.content)
        self.assertIn(b"13711001101",response.content)
...
class SignIndexActionTest(TestCase):
    ...
    def setUp(self):
        ...
        User.objects.create_user('admin', 'admin@mail.com', 'admin123456')
        Event.objects.create(id=1, name="xiaomi5", limit=2000, address='beijing', status=1, start_time='2017-8-10 12:30:00')
        Event.objects.create(id=2, name="oneplus4", limit=2000, address='guangzhou', status=1, start_time='2017-6-10 12:30:00')
        Guest.objects.create(id=1, event_id=1, realname='alen', phone='18611001100', email='alen@mail.com', sign=False)
        Guest.objects.create(id=2, event_id=2, realname='UNA', phone='18611001101', email='alen@mail.com', sign=True)
        self.login_user = {'username': 'admin', 'password': 'admin123456'}
    ...
    def test_sign_index_action_phone_null(self):
        ...
        b = self.assertIn(b"phone error",response.content)
